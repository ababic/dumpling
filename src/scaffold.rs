...
pub fn run_scaffold_config(opts: ScaffoldConfigOptions<'_>) -> anyhow::Result<()> {
    ...
    if rules.is_empty() {
        ...
        eprintln!(
            "dumpling scaffold-config: sampled up to {} row(s) per INSERT/COPY block for JSON path hints (--sample-rows)",
            sample_rows
        );
    }
...
# when generated with `--sample-rows` (dot-separated keys: `payload.profile.email`).
